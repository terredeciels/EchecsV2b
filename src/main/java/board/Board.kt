...
class Board : Constantes {
    ...
    fun isSquareAttacked(sqTarget: Int, side: Int): Boolean {
        return range(0, BOARD_SIZE)
            .filter { sq: Int -> squareColors[sq] == side }
            .anyMatch { sq: Int -> pieceAttacksSquare(sq, sqTarget, squarePieces[sq], side) }
    }
    ...
    fun attackViaOffset(sq: Int, sqTarget: Int, pieceType: Int, offsetIndex: Int): Boolean {
        ...
        while (mailbox[mailbox64[sqIndex] + offset[pieceType][offsetIndex]].also { sqIndex = it } != -1) {
            ...
            if (squareColors[sqIndex] != EMPTY || !slide[pieceType]) break
        }
        return false
    }
            ...
            enPassantSquare != -1 -> {
                ...
                offsets.forEach { offset ->
                    ...
                    if (enPassantSquare and 7 != (if (offset == offsets[0]) 0 else 7))
                        if (squareColors[newEp] == targetColor && squarePieces[newEp] == PAWN) addMove(newEp, enPassantSquare, 21)
                }
            }
        }
    ...
    fun generateCastlingMoves() {
        ...
        val (kingStart, kingsideTarget, towersideTarget) = if (currentSide == LIGHT) Triple(E1, G1, C1) else Triple(E8, G8, C8)
    ...
    fun makeMove(m: Move): Boolean {
        ...
        if (m.bits and 2 != 0) {
            ...
            when (m.to) {
                G1 -> {
                    if (squareColors[F1] != EMPTY || squareColors[G1] != EMPTY || isSquareAttacked(F1, opponentSide) || isSquareAttacked(
                            G1,
                            opponentSide
                        )
                    ) return false
                ...
                C1 -> {
                    if (squareColors[B1] != EMPTY || squareColors[C1] != EMPTY || squareColors[D1] != EMPTY || isSquareAttacked(
                            C1,
                            opponentSide
                        ) || isSquareAttacked(D1, opponentSide)
                    ) return false
                    from = A1
                    to = D1
                }
                ...
                G8 -> {
                    if (squareColors[F8] != EMPTY || squareColors[G8] != EMPTY || isSquareAttacked(F8, opponentSide) || isSquareAttacked(
                            G8,
                            opponentSide
                        )
                    ) return false
                ...
                C8 -> {
                    if (squareColors[B8] != EMPTY || squareColors[C8] != EMPTY || squareColors[D8] != EMPTY || isSquareAttacked(
                            C8,
                            opponentSide
                        ) || isSquareAttacked(D8, opponentSide)
                    ) return false
                    from = A8
                    to = D8
                }
        ...
        enPassantSquare = if (m.bits and 8 != 0) {
        ...
        // Déplacer la pièce
        setSquare(squareColors, squarePieces, m.to, currentSide, if ((m.bits and 32) != 0) m.promote else squarePieces[m.from])
    ...
    fun undoMove() {
        ...
        setSquare(squareColors, squarePieces, m.from, currentSide, if ((m.bits and 32) != 0) PAWN else squarePieces[m.to])
    ...
    fun isInCheck(board: Board, s: Int): Boolean {
        return range(0, BOARD_SIZE)
            .filter { i: Int -> board.squarePieces[i] == KING && board.squareColors[i] == s }
